...
export class ShipmentFormManager {
    ...
    if (this.currentStep === 1) {
      ...
      if (incoTerm === 'FOB') {
        if (!freightCharges || parseFloat(freightCharges) <= 0) {
          errors.push('Please enter valid Freight Charges (required for FOB inco-term)');
        }
      }
  ...
  populateDetailsReview() {
    ...
    if (incoTerm === 'FOB' && freightCharges) {
      html += `<div class="review-item"><span class="review-item-label">Freight Charges:</span><span class="review-item-value">$${parseFloat(freightCharges).toFixed(2)} USD</span></div>`;
    }
